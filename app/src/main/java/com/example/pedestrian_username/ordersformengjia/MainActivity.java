...
public class MainActivity extends RosActivity {
    ...
    // 语音对象
    private SpeechRecognizer mAsr_iat;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 初始化对象
        mAsr_iat = SpeechRecognizer.createRecognizer(MainActivity.this, mInitListener);
        ...
        asr_iat_tts_socket_button.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                flag = !flag;
                if(flag) {
                    mToast.setText("已切换到语音识别模式");
                    mToast.show();
                } else {
                    mToast.setText("已切换到语音听写模式");
                    mToast.show();
                }
                return true;
            }
        ...
        ret = mAsr_iat.buildGrammar("bnf", mContent, mLocalGrammarListener);
    ...
    public void asr() {
        asr_setParam();
        ret = mAsr_iat.startListening(asr_mRecognizerListener);
        if (ret != ErrorCode.SUCCESS) {
    ...
    private void iat(){
        ...
        mIatResults.clear();
        mAsr_iat = com.iflytek.cloud.SpeechRecognizer.createRecognizer(MainActivity.this, mInitListener);
        iat_setParam();
        // 不显示听写对话框
        ret = mAsr_iat.startListening(iat_mRecognizerListener);
        if (ret != ErrorCode.SUCCESS) {
    ...
    public void asr_setParam(){
        mAsr_iat.setParameter(SpeechConstant.PARAMS, null);
        mAsr_iat.setParameter(SpeechConstant.ENGINE_TYPE, SpeechConstant.TYPE_LOCAL);
        mAsr_iat.setParameter(SpeechConstant.RESULT_TYPE, "json");
        mAsr_iat.setParameter(SpeechConstant.LOCAL_GRAMMAR, "command");
        mAsr_iat.setParameter(SpeechConstant.ASR_THRESHOLD, "30");
        mAsr_iat.setParameter(SpeechConstant.AUDIO_FORMAT,"wav");
        mAsr_iat.setParameter(SpeechConstant.ASR_AUDIO_PATH, Environment.getExternalStorageDirectory()+"/讯飞语音平台/asr.wav");
    }
    ...
    public void iat_setParam() {
        ...
        mAsr_iat.setParameter(SpeechConstant.ENGINE_TYPE, SpeechConstant.TYPE_LOCAL);
        mAsr_iat.setParameter(SpeechConstant.RESULT_TYPE, "json");
        mAsr_iat.setParameter(SpeechConstant.LANGUAGE, "zh_cn");
        mAsr_iat.setParameter(SpeechConstant.ACCENT, "mandarin ");
        mAsr_iat.setParameter(SpeechConstant.VAD_BOS,"4000");
        mAsr_iat.setParameter(SpeechConstant.VAD_EOS,"1000");
        // 设置标点符号,设置为"0"返回结果无标点,设置为"1"返回结果有标点
        mAsr_iat.setParameter(SpeechConstant.ASR_PTT,"1");
        mAsr_iat.setParameter(SpeechConstant.AUDIO_FORMAT,"wav");
        mAsr_iat.setParameter(SpeechConstant.ASR_AUDIO_PATH, Environment.getExternalStorageDirectory()+"/讯飞语音平台/iat.wav");
    }
